...
required_frontend_cols_in_geojson = sorted(list(set([
    ...
    'ndi_o', 'uei_o', 'hoi_o', 'andi_final', 'auei_final', 'ahoi_final', 'top10_dests', 'top10_origins'
])))
...
# --- Flask Routes ---
# --- Route to Get Tract Connectivity Info ---
@app.route('/api/tract_connectivity/<tract_id>')
@login_required
def get_tract_connectivity(tract_id):
    # --- Authentication Check (if you have login) ---
    # if 'logged_in' not in session:
    #     return jsonify({"error": "Unauthorized"}), 401
    # --- End Auth Check ---

    mode = request.args.get('mode', 'destinations') # 'destinations' or 'origins'
    index_field = request.args.get('index_field', None) # Get the currently selected index/variable

    conn = None # Initialize conn to None
    try:
        conn = get_db_conn()
        # Build the query safely
        # Select basic info, the requested index field, and both top lists
        select_columns = [
            '"Origin_tract"',
            '"population_x_o"',
            '"race"',
        ...
        # Safely add the selected index field if provided and valid
        # **Important**: Add validation here to prevent SQL injection if index_field
        # comes directly from user input without sanitization/validation.
        # Assuming index_field corresponds to a known, safe column name.
        if index_field and isinstance(index_field, str) and index_field.isalnum(): # Basic validation
             select_columns.append(f'"{index_field}"')
        ...
        query = f"""
            ...
            LIMIT 1
        """
        # print(f"Executing query: {query} with tract_id: {tract_id}") # Debugging
        result = conn.execute(query, [tract_id]).fetch_df() # Fetch as Pandas DataFrame
        ...
        # Prepare basic info, handling potential missing columns/values
        clicked_tract_info = {
            "id": data.get("Origin_tract", tract_id),
            "population": data.get("population_x_o", "N/A"),
            "dominant_race": data.get("race", "N/A"),
            "poverty_rate": data.get("poverty_rate_o", "N/A"),
            "selected_index_value": data.get(index_field, "N/A") if index_field else "N/A"
        }

        # Parse the string list for the requested mode
        top_list_str = data.get("top10_dests", "[]") if mode == 'destinations' else data.get("top10_origins", "[]")
        top_list = parse_list_string(top_list_str)

        response_data = {
            "clicked_tract_info": clicked_tract_info,
            "top_list": top_list,
            "mode": mode # Echo back the mode
        }
        return jsonify(response_data)

    except Exception as e:
        print(f"Error in /api/tract_connectivity/{tract_id}: {e}") # Log the error
