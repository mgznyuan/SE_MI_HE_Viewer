...
let selectedTractId = null; // ID of the currently clicked tract
let connectionsMode = 'destinations'; // 'destinations' or 'origins'
let connectionsLayerGroup = L.layerGroup(); // To hold highlights and lines
let selectedTractHighlightLayer = null; // Specific layer for clicked tract border
let connectionLegend = null; // To hold the connection rank legend
...
// --- NEW Border Colors for Connections (Example) ---
const connectionBorderColors = [
    '#d73027', // Rank 1-2: Strong Red
    '#d73027',
    '#fc8d59', // Rank 3-4: Orange
    '#fc8d59',
    '#fee08b', // Rank 5-6: Yellow
    '#fee08b',
    '#91bfdb', // Rank 7-8: Light Blue
    '#91bfdb',
    '#4575b4', // Rank 9-10: Stronger Blue
    '#4575b4'
];
const connectionBorderWidths = [
    3, 3,     // Rank 1-2
    2.5, 2.5, // Rank 3-4
    2, 2,     // Rank 5-6
    1.5, 1.5, // Rank 7-8
...
const displayableVariables = {
  ...
  "Pre-defined Indices": [
    { id: "ndi_o", name: "Neighborhood Disadvantage Index (NDI)*100" },
    { id: "uei_o", name: "Unhealthy Exposure Index (UEI)*100" },
    { id: "hoi_o", name: "Health Opportunity Index (HOI)*100" },
    { id: "andi_final", name: "Activity Space Neighborhood Disadvantage Index (ANDI)*100" },
    { id: "auei_final", name: "Activity Space Unhealthy Exposure Index (AUEI)*100" },
    { id: "ahoi_final", name: "Activity Space Health Opportunity Index (AHOI)*100" }]
};
...
// --- Helper function to show tract connections ---
function getConnectionColor(rank) { // rank is 1-based
    if (rank >= 1 && rank <= connectionColors.length) {
        return connectionColors[rank - 1]; // Array is 0-indexed
    }
    return '#cccccc'; // Fallback color for invalid rank
}

function findFeatureById(tractId) {
    if (!data || !data.features) return null;
    return data.features.find(f => f.properties['Origin_tract'] === tractId);
}
...
// --- Function to show activity space---
// --- REVISED updateSelectedTractInfo using a TABLE ---
function updateSelectedTractInfo(tractId) {
    const infoPanel = document.getElementById('selected-tract-info');
    if (!infoPanel) {
        console.error("Element #selected-tract-info not found!");
        return;
    }

    const feature = findFeatureById(tractId);
    if (!feature) {
        infoPanel.innerHTML = `Details for tract ${tractId} not found.`;
        return;
    }
    ...
    // --- Prepare Data ---
    const population = props['population_x_o'] !== undefined ? parseFloat(props['population_x_o']).toLocaleString() : 'N/A';
    const poverty = props['poverty_rate_o'] !== undefined ? `${parseFloat(props['poverty_rate_o']).toFixed(1)}%` : 'N/A';
    const race = formatRaceName(props['race']) || 'N/A';
    const countyFips = props['COUNTYFP'];
    const countyName = countyFips ? (countyMapping[countyFips] || `Unknown (${countyFips})`) : 'N/A';

    let activeValueFormatted = 'N/A';
    let activeFieldNameFriendly = 'N/A';
    if (activeField && props.hasOwnProperty(activeField)) {
         const activeValueRaw = props[activeField];
         activeValueFormatted = (activeValueRaw === null || activeValueRaw === undefined || isNaN(parseFloat(activeValueRaw)))
                                 ? 'N/A'
                                 : parseFloat(activeValueRaw).toFixed(2);
         activeFieldNameFriendly = getFriendlyFieldName(activeField);
    } else if (activeField) {
        activeFieldNameFriendly = getFriendlyFieldName(activeField);
        activeValueFormatted = 'N/A';
    }

    // --- Build HTML Table String ---
    let html = `<strong>Selected Tract: ${tractId}</strong>`;
    // Start table - add a class for potential styling
    html += `<table class="selected-tract-table">`;
    // Use tbody for content rows
    html += `<tbody>`;
    // Add rows using <tr> for row, <td> for cells
    html += `<tr><td>County:</td><td>${countyName}</td></tr>`;
    html += `<tr><td>Dominant Race:</td><td>${race}</td></tr>`;
    html += `<tr><td>Population:</td><td>${population}</td></tr>`;
    html += `<tr><td>Poverty Rate:</td><td>${poverty}</td></tr>`;
    // Add row for the active variable/index if one is selected
    if (activeField) {
        html += `<tr><td>${activeFieldNameFriendly}:</td><td>${activeValueFormatted}</td></tr>`;
    }
    // Close table body and table
    html += `</tbody>`;
    html += `</table>`;
    // --- End HTML Table String ---

    infoPanel.innerHTML = html;
    console.log("Updated #selected-tract-info with table.");
}


function clearConnectionsDisplay() {
    console.log("Clearing connections display.");
    if (connectionsLayerGroup) {
        map.removeLayer(connectionsLayerGroup); // Remove from map
        connectionsLayerGroup.clearLayers(); // Clear internal layers
    }
    ...
    const infoPanel = document.getElementById('selected-tract-info');
    if (infoPanel) {
        infoPanel.innerHTML = 'Click a tract on the map to view its details and connections.';
    }
    const tableButton = document.getElementById('show-connections-table');
    if (tableButton) {
        tableButton.disabled = true;
    }
    // Optional: Reset button active state if needed, though typically
    // the user expectation is it remembers the last mode.
    // connectionsMode = 'destinations'; // Reset to default?
    // document.getElementById('show-destinations').classList.add('active-connection-mode');
    // document.getElementById('show-origins').classList.remove('active-connection-mode');
}

// --- Add this new function ---
function addConnectionLegend() {
    // Remove existing connection legend if it exists
    removeConnectionLegend(); // We'll define this helper next

    // --- Check map validity ---
    if (!isValidMap(map)) {
        console.error("Cannot add connection legend: Leaflet map object is invalid.");
        return;
    }

    connectionLegend = L.control({ position: 'topright' }); // Or 'bottomleft' etc.

    connectionLegend.onAdd = function (map) {
        const div = L.DomUtil.create('div', 'info legend connection-legend');
        div.innerHTML += `<strong>Connection Rank</strong><br>`;
        div.innerHTML += `(<small>${connectionsMode === 'destinations' ? 'Destinations' : 'Origins'}</small>)<br>`; // Show current mode

        // Define legend labels and pair them with colors/widths
        const legendItems = [
            { label: '1 - 2', rank: 1 },
            { label: '3 - 4', rank: 3 },
            { label: '5 - 6', rank: 5 },
        ...
        legendItems.forEach(item => {
            const color = connectionBorderColors[item.rank - 1];
            const width = connectionBorderWidths[item.rank - 1];
            // Style the icon `<i>` to look like a border swatch
            div.innerHTML +=
                `<div><i style="background: none; border: ${width}px solid ${color}; width: 18px; height: ${18 - (2*width)}px; display: inline-block; margin-right: 5px; vertical-align: middle;"></i> ${item.label}</div>`;
        });
        div.innerHTML += '<div style="margin-top: 5px;"><i class="selected-tract-legend-swatch" style="background: none; border: 3px solid #e63946; width: 18px; height: 12px; display: inline-block; margin-right: 5px; vertical-align: middle;"></i> Selected Tract</div>'; // Add swatch for selected tract

        return div;
    };

    connectionLegend.addTo(map);
    console.log("Added connection legend control.");
}

function removeConnectionLegend() {
    if (isValidMap(map) && connectionLegend) {
        try {
             map.removeControl(connectionLegend);
             console.log("Removed connection legend control.");
        } catch (e) {
             console.error("Error removing connection legend control:", e);
        }
    }
    connectionLegend = null; // Reset the variable
}

function displayConnections() {
    // --- Initial checks ---
    console.log("--- displayConnections called ---"); // Log Entry
    if (!selectedTractId || !connectionsMode) {
        console.warn("displayConnections stopped: No selectedTractId or connectionsMode.");
        return;
    }
    if (!data || !data.features) {
        console.error("displayConnections stopped: GeoJSON data not loaded.");
        clearConnectionsDisplay(); // Clear just in case
        return;
    }
    console.log(`Mode: ${connectionsMode}, Target Tract: ${selectedTractId}`);

    // --- Clear previous layers & legend ---
    console.log("Clearing connectionsLayerGroup and legend...");
    removeConnectionLegend(); // Remove previous connection legend first
    connectionsLayerGroup.clearLayers();
    selectedTractHighlightLayer = null;

    // --- Get Selected Feature ---
    console.log(`Finding feature for ID: ${selectedTractId}`);
    const selectedFeature = findFeatureById(selectedTractId);
    if (!selectedFeature) {
        console.error(`Selected tract feature not found: ${selectedTractId}`);
        clearConnectionsDisplay(); // Clear if feature not found
        alert(`Data for selected tract ${selectedTractId} could not be found.`);
        return; // Stop execution
    }
    console.log("Selected feature found:", selectedFeature.properties); // Log found properties

    // --- Get Connection String ---
    const propName = connectionsMode === 'destinations' ? 'top10_dests' : 'top10_origins';
    console.log(`Attempting to get property: ${propName}`);
    const connectionsString = selectedFeature.properties[propName];
    console.log(`>>> DEBUG: Raw connectionsString for ${selectedTractId} / ${propName} is: [${connectionsString}] (Type: ${typeof connectionsString})`);

    let topTractIds = []; // Initialize

    // **** NEW ROBUST Custom Parsing Block ****
    if (connectionsString && typeof connectionsString === 'string' && connectionsString.trim().length > 0) {
        console.log("Entering custom string parsing block...");
        try {
            // 1. Trim whitespace from the entire string
            let processedString = connectionsString.trim();
            console.log("Parse Step 0: Trimmed:", `[${processedString}]`);

            // 2. Remove potential leading/trailing brackets `[` `]`
            if (processedString.startsWith('[') && processedString.endsWith(']')) {
             ...
             console.log("Parse Step 1: Brackets Removed:", `[${processedString}]`);

            // 3. Remove all single quotes `'` globally
            processedString = processedString.replace(/'/g, '');
             console.log("Parse Step 2: Single Quotes Removed:", `[${processedString}]`);

            // 4. Replace newline characters `\n` and potentially other whitespace runs with a single space
            processedString = processedString.replace(/\s+/g, ' ').trim(); // Replace any whitespace sequence with ONE space
             console.log("Parse Step 3: Whitespace Normalized:", `[${processedString}]`);

            // 5. Split the resulting string by the single space delimiter
            const splitItems = processedString.split(' '); // Split by single space
             console.log("Parse Step 4: Split Items:", splitItems);

             // 6. Map, Trim, and Filter
            topTractIds = splitItems
                            .map(item => item.trim()) // Trim each item
                            .filter(item => { // Filter with logging
                                const isEmpty = !item;
                                const isDigits = /^\d+$/.test(item); // Assumes tract IDs are digits only
                                console.log(`Filtering item: '[${item}]', isEmpty: ${isEmpty}, isDigits: ${isDigits}, Keep: ${!isEmpty && isDigits}`);
                                return !isEmpty && isDigits; // Keep non-empty, digit-only strings
                            });

            console.log("Parse Step 5: Filtered Result (topTractIds):", topTractIds);

            // Optional: Limit to top 10
            if (topTractIds.length > 10) {
                console.warn("Parsing yielded more than 10 IDs, taking first 10.");
                topTractIds = topTractIds.slice(0, 10);
            }

        } catch (error) {
            console.error(`Error during robust custom parsing of ${propName} string for tract ${selectedTractId}:`, error);
            console.error("Original string was:", connectionsString);
            topTractIds = []; // Ensure it's empty on error
        }
    } else if (Array.isArray(connectionsString)) {
         // Handle case where it might already be an array
         console.log(`Property ${propName} for tract ${selectedTractId} is already an array.`);
         topTractIds = connectionsString;
         // Ensure items are strings and trim/filter just in case
         topTractIds = topTractIds.map(item => String(item).trim()).filter(item => item && /^\d+$/.test(item));
         if (topTractIds.length > 10) { topTractIds = topTractIds.slice(0, 10); }

    } else {
         console.warn(`Property ${propName} for tract ${selectedTractId} is missing, null, or not a processable string/array. Value: [${connectionsString}]`);
         topTractIds = []; // Ensure empty array
    }
    // **** END NEW ROBUST Custom Parsing Block ****


    // --- ALWAYS Highlight the Selected Tract ---
    console.log("Highlighting selected tract (border):", selectedTractId);
    try {
        selectedTractHighlightLayer = L.geoJson(selectedFeature, {
            style: {
                // Style with border, no fill for selected tract
                fillOpacity: 0,
                color: '#e63946', // Distinct color for selected (e.g., red)
                weight: 3,        // Make selected border thicker
                opacity: 1,
                className: 'selected-tract-highlight' // Add class if needed
            },
            pane: 'shadowPane' // Ensure it's drawn correctly relative to base layer
        }).addTo(connectionsLayerGroup);
    } catch (e) {
        console.error("Error creating selected tract highlight layer:", e);
    }
    // --- END Highlight Selected Tract ---


    // --- Check if connections were found AFTER parsing ---
    console.log(`>>> FINAL CHECK: topTractIds type: ${typeof topTractIds}, isArray: ${Array.isArray(topTractIds)}, length: ${topTractIds ? topTractIds.length : 'N/A'}`);
    if (!topTractIds || topTractIds.length === 0) {
        console.warn(">>> FINAL RESULT: No valid connection IDs found. Only showing selected tract highlight.");
        map.addLayer(connectionsLayerGroup); // Add group (highlight only)
        alert(`No valid top ${connectionsMode} data found for the selected tract.`); // Alert user
        const tableButton = document.getElementById('show-connections-table');
        if (tableButton) tableButton.disabled = true; // Ensure table button is disabled
        // DO NOT add connection legend if no connections found
        return; // STOP HERE
    }
    // --- END Check for connections ---


    // --- If connections WERE found, proceed ---
    console.log(">>> FINAL RESULT: Connections found! Proceeding to highlight borders.");

    // --- Iterate and Highlight Top Connections (using BORDERS) ---
    console.log(`Iterating through ${topTractIds.length} connection IDs...`);
    topTractIds.forEach((tractId, index) => {
        const currentTractId = String(tractId).trim(); // Ensure string and trim
        console.log(`Loop Rank ${index + 1}: Processing ID '${currentTractId}'`);
        if (!currentTractId) {
             console.log(`Loop Rank ${index + 1}: Skipping empty ID.`);
             return; // Skip empty IDs
        }

        // Do not highlight if it's the same as the selected tract (already highlighted)
        if (currentTractId === selectedTractId) {
            console.log(`Loop Rank ${index + 1}: Skipping ID '${currentTractId}' because it's the selected tract.`);
            return;
        }

        const connectedFeature = findFeatureById(currentTractId);
        if (!connectedFeature) {
            console.warn(`Loop Rank ${index + 1}: Connected feature NOT FOUND for ID: '${currentTractId}'`);
            return; // Skip if feature not in main dataset
        }
        console.log(`Loop Rank ${index + 1}: Connected feature FOUND for ID: '${currentTractId}'`);

        const rank = index + 1;
        // Get border color and width based on rank
        const borderColor = connectionBorderColors[rank - 1] || '#cccccc'; // Fallback grey
        const borderWidth = connectionBorderWidths[rank - 1] || 1; // Fallback width

        // Create highlight layer for the connected tract - STYLE FOR BORDER
        try {
            const highlightLayer = L.geoJson(connectedFeature, {
                style: {
                    fillOpacity: 0,     // NO fill for connected tracts
                    color: borderColor, // Set BORDER color from rank
                    weight: borderWidth,// Set BORDER width from rank
                    opacity: 0.9        // Border opacity
                },
                pane: 'overlayPane' // Render above base choropleth but potentially below selected highlight
            }).bindTooltip(`Rank: ${rank}<br>Tract: ${currentTractId}<br>(${connectionsMode === 'destinations' ? 'Destination' : 'Origin'})`);
            highlightLayer.addTo(connectionsLayerGroup);
            console.log(`Loop Rank ${index + 1}: Added BORDER highlight layer for '${currentTractId}'`);
        } catch (e) {
            console.error(`Loop Rank ${index + 1}: Error creating BORDER highlight layer for connected tract ${currentTractId}:`, e);
        }

        // --- Connecting lines and self-markers are removed for now ---

    }); // End forEach
    console.log("Finished iterating through connection IDs.");

    // Add the layer group (highlight + connection borders) to the map
    console.log("Adding connectionsLayerGroup to map.");
    map.addLayer(connectionsLayerGroup);

    // Enable the table button because connections were found
    const tableButton = document.getElementById('show-connections-table');
    if (tableButton) {
        console.log("Enabling connections table button.");
        tableButton.disabled = false;
    }

    // Add the connection legend
    addConnectionLegend();

    console.log("--- displayConnections finished ---");

} // --- End of displayConnections function ---



// /**
//  * Displays highlights for the selected tract and its top connections.
//  * Uses border colors/widths for connected tracts based on rank.
//  * Adds arrows pointing from selected tract centroid to connected tract centroid.
//  */
// function displayConnections() {
//     // --- Initial checks ---
//     console.log("--- displayConnections called ---");
//     if (!selectedTractId || !connectionsMode) { /* ... */ return; }
...
//     console.log(`Finding feature for ID: ${selectedTractId}`);
//     const selectedFeature = findFeatureById(selectedTractId);
//     if (!selectedFeature) { /* ... handle error ... */ return; }
//     console.log("Selected feature found.");

//     // --- Get Selected Tract Centroid (calculate ONCE) ---
//     let selectedCenter = null;
//     try {
//         // Use L.geoJSON to easily get bounds and center
//         selectedCenter = L.geoJson(selectedFeature).getBounds().getCenter();
//         console.log("Selected tract center:", selectedCenter);
//     } catch (e) {
//         console.error("Could not get center for selected tract:", selectedTractId, e);
//         // Proceed without arrows if center calculation fails
//     }
//     // --- END Get Selected Tract Centroid ---

//     // --- Get Connection String & Parse ---
//     const propName = connectionsMode === 'destinations' ? 'top10_dests' : 'top10_origins';
//     const connectionsString = selectedFeature.properties[propName];
//     let topTractIds = [];
//     // **** Robust Custom Parsing Block (Keep from previous step) ****
//     if (connectionsString && typeof connectionsString === 'string' && connectionsString.trim().length > 0) {
//         try {
//             let processedString = connectionsString.trim();
//             if (processedString.startsWith('[') && processedString.endsWith(']')) { processedString = processedString.substring(1, processedString.length - 1); }
//             processedString = processedString.replace(/'/g, '');
//             processedString = processedString.replace(/\s+/g, ' ').trim();
//             topTractIds = processedString.split(' ').map(item => item.trim()).filter(item => item && /^\d+$/.test(item));
//             if (topTractIds.length > 10) { topTractIds = topTractIds.slice(0, 10); }
//              console.log("Custom parsing result:", topTractIds);
//         } catch (error) { console.error(`Error parsing ${propName}:`, error); topTractIds = []; }
//     } else if (Array.isArray(connectionsString)) {
//          topTractIds = connectionsString.map(item => String(item).trim()).filter(item => item && /^\d+$/.test(item));
//          if (topTractIds.length > 10) { topTractIds = topTractIds.slice(0, 10); }
//     } else {
//          console.warn(`Property ${propName} missing or invalid.`); topTractIds = [];
//     }
//     // **** END Parsing Block ****
...
//     console.log("Highlighting selected tract (border):", selectedTractId);
//     try {
//         selectedTractHighlightLayer = L.geoJson(selectedFeature, { /* ... style ... */ }).addTo(connectionsLayerGroup);
//     } catch (e) { console.error("Error creating selected tract highlight layer:", e); }

//     // --- Check if connections were found AFTER parsing ---
//     console.log(`>>> FINAL CHECK: topTractIds length: ${topTractIds ? topTractIds.length : 'N/A'}`);
//     if (!topTractIds || topTractIds.length === 0) {
//         console.warn(">>> FINAL RESULT: No valid connection IDs found. Only showing selected tract highlight.");
//         map.addLayer(connectionsLayerGroup); // Add group (highlight only)
//         alert(`No valid top ${connectionsMode} data found for the selected tract.`);
//         const tableButton = document.getElementById('show-connections-table');
...
//     // --- If connections WERE found, proceed ---
//     console.log(">>> FINAL RESULT: Connections found! Proceeding to highlight borders & add arrows.");

//     // --- Iterate, Highlight Borders, AND Add Arrows ---
//     console.log(`Iterating through ${topTractIds.length} connection IDs...`);
//     topTractIds.forEach((tractId, index) => {
//         const currentTractId = String(tractId).trim();
//         if (!currentTractId) return; // Skip empty
...
//         // --- Highlight Connected Tract Border (Skip if same as selected) ---
//         if (currentTractId !== selectedTractId) {
//             const borderColor = connectionBorderColors[rank - 1] || '#cccccc';
//             const borderWidth = connectionBorderWidths[rank - 1] || 1;
//             try {
//                 const highlightLayer = L.geoJson(connectedFeature, {
//                     style: { fillOpacity: 0, color: borderColor, weight: borderWidth, opacity: 0.9 },
//                     pane: 'overlayPane'
//                 }).bindTooltip(/* ... tooltip ... */);
//                 highlightLayer.addTo(connectionsLayerGroup);
//             } catch (e) { console.error(`Loop Rank ${rank}: Error creating BORDER highlight layer for ${currentTractId}:`, e); }
//         }
//         // --- END Border Highlight ---

//         // --- Add Arrow (if different tract and centroids are valid) ---
//         if (currentTractId !== selectedTractId && selectedCenter) {
//             let connectedCenter = null;
//             try {
//                 connectedCenter = L.geoJson(connectedFeature).getBounds().getCenter();
//             } catch (e) {
//                 console.warn(`Loop Rank ${rank}: Could not get center for connected tract ${currentTractId}:`, e);
//             }

//             // Only draw if both centers are valid
//             if (connectedCenter) {
//                 // Create the line segment (can be invisible or styled subtly)
//                 const baseLine = L.polyline([selectedCenter, connectedCenter], {
//                     // Example subtle style, or make opacity 0 if decorator is enough
...
//                 // Check if decorator plugin is loaded
//                 if (L.polylineDecorator) {
//                     // Add the decorator with arrowhead
//                     const arrow = L.polylineDecorator(baseLine, {
//                         patterns: [
//                             {
//                                 offset: '100%', // Place arrowhead at the end of the line
//                                 repeat: 0,      // Only draw one arrowhead
//                                 symbol: L.Symbol.arrowHead({
//                                     pixelSize: 12,          // Adjust size
//                                     polygon: true,         // Make it a filled arrow
//                                     pathOptions: {
//                                         stroke: true,
//                                         weight: 1,
//                                         color: '#2a9d8f',    // Arrow border color (e.g., teal)
//                                         fillColor: '#2a9d8f', // Arrow fill color (e.g., teal)
//                                         fillOpacity: 0.6,    // Arrow fill opacity
//                                         opacity: 0.7         // Arrow border opacity
//                                     }
//                                 })
//                             }
//                         ]
//                     }).addTo(connectionsLayerGroup);
//                     console.log(`Loop Rank ${rank}: Added arrow decorator to '${currentTractId}'`);
//                 } else {
//                     console.warn("Leaflet.PolylineDecorator plugin not loaded. Cannot draw arrows.");
//                 }
//             } // end if connectedCenter
//         }
//         // --- END Add Arrow ---

//          // --- Handle Self-Connection Marker (Optional) ---
//          else if (currentTractId === selectedTractId && selectedCenter) {
//               // If the top connection is the tract itself, maybe add a special marker
//               try {
//                   L.circleMarker(selectedCenter, { /* ... self-marker style ... */ })
//                    .bindTooltip(`Rank: ${rank}<br>Tract: ${currentTractId}<br>(Origin/Destination is Self)`)
//                    .addTo(connectionsLayerGroup);
//                   console.log(`Loop Rank ${rank}: Added self-marker for '${currentTractId}'`);
//               } catch (e) { console.warn(`Could not draw self-marker for tract ${currentTractId}:`, e); }
//          }
//          // --- END Self-Connection Marker ---


//     }); // End forEach
//     console.log("Finished iterating through connection IDs.");

//     // --- Add Layer Group and Legend ---
//     console.log("Adding connectionsLayerGroup to map.");
//     map.addLayer(connectionsLayerGroup);
//     const tableButton = document.getElementById('show-connections-table');
//     if (tableButton) { tableButton.disabled = false; }
//     addConnectionLegend(); // Add legend now that processing is done

//     console.log("--- displayConnections finished ---");

// } // --- End of displayConnections function ---
// --- Add this function to show connection table---
// --- UPDATED showConnectionsTable Function ---
function showConnectionsTable() {
    console.log("--- showConnectionsTable called ---")
    if (!selectedTractId || !connectionsMode) {
        alert("Please select a tract and a connection mode first.");
        return;
    }

    const selectedFeature = findFeatureById(selectedTractId);
    if (!selectedFeature) {
        alert(`Selected tract data not found (${selectedTractId}).`);
        return;
    }

    const propName = connectionsMode === 'destinations' ? 'top10_dests' : 'top10_origins';
    const connectionsString = selectedFeature.properties[propName];
    console.log(`Table: Raw value for ${propName}:`, connectionsString);

    let topTractIds = []; // Initialize

    // **** Use the SAME Robust Custom Parsing Block ****
    if (connectionsString && typeof connectionsString === 'string' && connectionsString.trim().length > 0) {
        try {
            let processedString = connectionsString.trim();
            if (processedString.startsWith('[') && processedString.endsWith(']')) { processedString = processedString.substring(1, processedString.length - 1); }
            ...
            processedString = processedString.replace(/\s+/g, ' ').trim();
            topTractIds = processedString.split(' ').map(item => item.trim()).filter(item => item && /^\d+$/.test(item));
            console.log("Table: Custom parsing result:", topTractIds);
            if (topTractIds.length > 10) { topTractIds = topTractIds.slice(0, 10); }
        } catch (error) { console.error(`Table: Error parsing ${propName}:`, error); topTractIds = []; }
    } else if (Array.isArray(connectionsString)) {
        topTractIds = connectionsString;
    } else {
        console.warn(`Table: Property ${propName} missing or invalid.`); topTractIds = [];
    }
    // **** END Parsing Block ****
    ...
    closeFloatingPanel(); // Close any previous output
    panel.classList.remove('hidden');
    panelTitle.innerHTML = `Top 10 ${connectionsMode === 'destinations' ? 'Destinations' : 'Origins'} for Tract ${selectedTractId}`;
    output.innerHTML = `<h4>Top 10 ${connectionsMode === 'destinations' ? 'Destinations' : 'Origins'} for ${selectedTractId}</h4>`;


    if (!topTractIds || topTractIds.length === 0) {
         console.log("Table: No connection IDs to display.");
        output.innerHTML += `<p>No top ${connectionsMode} data available for this tract (or data format error).</p>`;
        return;
    }

    // --- Build Table ---
    console.log("Table: Building table for", topTractIds.length, "IDs");
    const table = document.createElement('table');
    table.className = 'data-table connections-table'; // Style the table

    // Header
    const thead = table.createTHead();
    const headerRow = thead.insertRow();
    ['Rank', 'Tract ID', 'County'].forEach(text => {
    ...
    topTractIds.forEach((tractId, index) => {
        const rank = index + 1;
        const currentTractId = String(tractId).trim(); // Use consistent variable name
        if (!currentTractId) return; // Skip empty

        const connectedFeature = findFeatureById(currentTractId);
        let countyName = 'N/A';
        if (connectedFeature) {
             const countyFips = connectedFeature.properties['COUNTYFP'];
             countyName = countyFips ? (countyMapping[countyFips] || `Unknown (${countyFips})`) : 'N/A';
        } else {
             countyName = '(Tract data missing)';
             console.warn(`Table: Feature data missing for tract ID: ${currentTractId}`);
        }

        const row = tbody.insertRow();

        // Rank Cell with Color Swatch (Border Style)
        ...
        swatch.style.marginLeft = '5px'; // Swatch after number
        swatch.style.verticalAlign = 'middle';
        swatch.style.borderWidth = `${connectionBorderWidths[rank-1] || 1}px`;
        swatch.style.borderStyle = 'solid';
        swatch.style.borderColor = connectionBorderColors[rank-1] || '#ccc';
        rankCell.appendChild(swatch); // Add swatch after rank number

        row.insertCell().textContent = currentTractId;
        row.insertCell().textContent = countyName;
    }); // End forEach

    output.appendChild(table);
    console.log("Table: Table added to output panel.");
}

// --- Function to generate and download Index Data CSV (SIMPLIFIED & FIXED) ---
function downloadIndexDataCSV(indexType) { // indexType = 'residential' or 'activity'
    const metadata = generatedIndexMetadata[indexType];

    // --- Validation using metadata.fieldName ---
    // Check if metadata exists AND if metadata.fieldName exists and is not empty
    if (!metadata || !metadata.fieldName || !data || !data.features) {
        alert(`Cannot download data. ${indexType.charAt(0).toUpperCase() + indexType.slice(1)} Index (field: ${metadata.fieldName || 'unknown'}) has not been generated or data is missing.`);
        return;
    }

    // Use the confirmed field name from metadata
    const confirmedIndexFieldName = metadata.fieldName;

    console.log(`Preparing SIMPLIFIED download for ${indexType} index: ${confirmedIndexFieldName}`);
    ...
    csvContent += `# Field ID: ${confirmedIndexFieldName}\n`; // Use confirmed name
    csvContent += `# Description: ${metadata.description || '(Not Set)'}\n`;
    csvContent += `# Variables Used in Calculation: ${metadata.variables.join(', ')}\n`;
    csvContent += `# --------------------------------------------------\n\n`;

    // --- Prepare CSV Header (Simplified) ---
    const headers = [
        'Origin_tract',
        'COUNTYFP',
        'CountyName',
        'population_x_o', // Verify this property name exists in your data.features[0].properties
        'race',           // Verify this property name exists
        confirmedIndexFieldName // Use confirmed name
    ];
    ...
    data.features.forEach(feature => {
        ...
        const countyFips = props['COUNTYFP'];
        row.push(countyFips || '');
        row.push(countyFips ? (countyMapping[countyFips] || '') : '');
        row.push(props['population_x_o'] || '');
        row.push(props['race'] || '');

        // Add ONLY the index score using the confirmed field name
        const indexValue = props.hasOwnProperty(confirmedIndexFieldName)
                           ? props[confirmedIndexFieldName]
                           : '';
        row.push(indexValue !== undefined && indexValue !== null ? indexValue : '');

        csvContent += row.map(escapeCSV).join(',') + '\n';
    });

    // --- Trigger Download ---
    // **** FIX: Use the confirmedIndexFieldName variable defined above ****
    const safeFieldName = confirmedIndexFieldName.replace(/[^a-zA-Z0-9_-]/g, '_').replace(/_+/g, '_');
...
async function generateLayerFromField(field, returnOnly = false) {
    ...
    if (!values.length) {
        ...
        if (!returnOnly) {
            ...
            // Create a gray layer indicating no data
            geojsonLayer = L.geoJson(data, {
                style: { fillColor: '#ccc', weight: 0.5, color: 'white', fillOpacity: 0.7 }, // Style for no data
                onEachFeature: (feature, layer) => {
                    // Add basic tooltip even for no-data layer
                    const props = feature.properties;
                    const tractId = props['Origin_tract'] || 'N/A';
                    const population = props['population_x_o'] !== undefined ? parseFloat(props['population_x_o']).toLocaleString() : 'N/A';
                    const countyFips = props['COUNTYFP'];
                    const countyName = countyFips ? (countyMapping[countyFips] || `Unknown (${countyFips})`) : 'N/A';
                    layer.bindTooltip(`<strong>Tract:</strong> ${tractId}<br><strong>County:</strong> ${countyName}<br><strong>Population:</strong> ${population}<br><strong>${getFriendlyFieldName(field)}:</strong> No Numeric Data`);

                    // **** STILL ADD CLICK LISTENER even on no-data layer ****
                    // So users can click to see connections regardless of the base layer's data
                    layer.on('click', (e) => {
                        const clickedLayer = e.target;
                        const clickedTractId = clickedLayer.feature.properties['Origin_tract'];
                        if (!document.getElementById('map-wrapper').classList.contains('hidden')) { return; } // Ignore in compare view
                        if (clickedTractId) {
                            selectedTractId = clickedTractId;
                            updateSelectedTractInfo(selectedTractId);
                            displayConnections();
                        } else {
                            console.error("Clicked feature missing 'Origin_tract' property.");
            ...
            if (legend) {
                 try { map.removeControl(legend); } catch (e) { console.warn("Could not remove legend for no-data layer:", e); }
            }
            activeField = field;
            document.getElementById('index-info').innerHTML = `Displaying: <strong>${getFriendlyFieldName(field)}</strong> (No numeric data found)`;
        }
        // Disable map download on no data
        if (mapDownloadButton) mapDownloadButton.disabled = true;
        return null; // Stop processing since there's no data to classify
    }
    // --- End Handle No Numeric Data ---


    // Calculate Jenks breaks using simple-statistics
    let breaks;
    try {
        const ss = await import('https://cdn.skypack.dev/simple-statistics'); // Import inside try block
        const uniqueValues = [...new Set(values)];
        const numClasses = 5; // Target 5 classes (yields 4 breaks / 6 boundary values from jenks)
        const actualNumClasses = Math.min(numClasses, uniqueValues.length);

        if (actualNumClasses < 2) {
            console.warn(`Only ${uniqueValues.length} unique value(s) for field ${field}. Cannot calculate Jenks breaks. Using simple range.`);
            // Create breaks array representing a single class range [min, max]
            breaks = [Math.min(...values), Math.max(...values)];
        } else {
            // Jenks returns k+1 values (class boundaries including min and max)
            breaks = ss.jenks(values, actualNumClasses);
            console.log(`Jenks breaks for ${field} (${actualNumClasses} classes):`, breaks);
        }
    } catch (jenksError) {
        console.error(`Jenks calculation failed for field ${field}:`, jenksError);
        if (!returnOnly) {
            // Display gray layer on error
            if (geojsonLayer && map.hasLayer(geojsonLayer)) geojsonLayer.remove();
            geojsonLayer = L.geoJson(data, { style: { fillColor: '#ccc', weight: 0.5, color: 'white', fillOpacity: 0.7 }, onEachFeature: (feature, layer) => {/* Basic tooltip or placeholder? */} }).addTo(map);
            if (legend) { try { map.removeControl(legend); } catch (e) {} } // Remove legend on error too
    ...
    const layer = L.geoJson(data, {
        // Style based on calculated breaks
        style: feature => ({
            fillColor: getColor(parseFloat(feature.properties[field]), breaks),
            weight: 0.5, // Base weight for all tracts in this layer
        ...
        onEachFeature: (feature, layer) => {
            const props = feature.properties;
            const tractId = props['Origin_tract'] || 'N/A'; // Get tract ID for tooltip and click
            ...
            const population = (populationRaw !== null && populationRaw !== undefined && !isNaN(parseFloat(populationRaw)))
                                 ? parseFloat(populationRaw).toLocaleString()
                                 : 'N/A';
            const countyFips = props['COUNTYFP'];
            const countyName = countyFips ? (countyMapping[countyFips] || `Unknown (${countyFips})`) : 'N/A';
            ...
            tooltipContent += `<strong>${friendlyFieldName}:</strong> ${valueFormatted}<br>`; // Display current field value
            tooltipContent += `<em>Click to see connections</em>`; // Add hint
            layer.bindTooltip(tooltipContent);
            // --- End Tooltip Content ---

            // **** CORRECTED Click Listener ****
            layer.on('click', (e) => {
                 const clickedLayer = e.target; // The specific feature layer that was clicked
                 const clickedTractId = clickedLayer.feature.properties['Origin_tract'];

                 // Prevent interaction if in comparison view
                 if (!document.getElementById('map-wrapper').classList.contains('hidden')) {
                     console.log("Map click ignored during comparison view.");
                     return;
                 }

                 // If a valid tract ID was found on the clicked feature
                 if (clickedTractId) {
                     selectedTractId = clickedTractId; // Update the global variable tracking the selection
                     updateSelectedTractInfo(selectedTractId); // Update the sidebar info panel
                     displayConnections(); // Trigger the display of highlights/lines based on current mode
                 } else {
                     console.error("Clicked feature missing 'Origin_tract' property.");
                     clearConnectionsDisplay(); // Clear any previous selection if the click is invalid
                 }
                 L.DomEvent.stopPropagation(e); // Stop the click event from propagating to the map itself
            });
            // **** END CORRECTED Click Listener ****
        }
    }); // End of L.geoJson creation
    ...
    if (geojsonLayer && map.hasLayer(geojsonLayer)) { // Check if layer exists and is on map
        geojsonLayer.remove(); // Remove the old layer
    }
    geojsonLayer = layer.addTo(map); // Add the new layer and store it
    // map.fitBounds(geojsonLayer.getBounds()); // Optionally zoom to the new layer's extent

    // --- Update Legend ---
    // Remove previous legend safely
    if (legend) { // Check if the legend object itself exists
        try {
            map.removeControl(legend); // Attempt to remove it, Leaflet handles if it's not there
        } catch (e) {
             console.warn("Could not remove previous legend control:", e);
        }
    }

    // Recreate legend content based on the NEW breaks
    legend.onAdd = function (mapInstance) {
        ...
        // Ensure breaks is valid and has enough elements for display
        if (breaks && breaks.length > 1) {
             if (breaks.length === 2) {
                 // Handle single class case (min/max range)
                 div.innerHTML += `<i style="background:${getColor(breaks[0] + (breaks[1]-breaks[0])*0.01, breaks)}"></i> ${breaks[0].toFixed(1)} – ${breaks[1].toFixed(1)}<br>`;
             } else {
                 // Standard Jenks breaks display (breaks = [min, b1, b2, ..., max])
                 // Class 1: <= b1
                 div.innerHTML += `<i style="background:${getColor(breaks[0], breaks)}"></i> ≤ ${breaks[1].toFixed(1)}<br>`;
                 // Class 2 to n-1: b(i-1) to bi
                 for (let i = 1; i < breaks.length - 1; i++) {
                      // Get color slightly above the lower break value for the swatch
                      const colorValue = breaks[i] + (breaks[i+1] - breaks[i]) * 0.01;
                      div.innerHTML += `<i style="background:${getColor(colorValue, breaks)}"></i> ${breaks[i].toFixed(1)} – ${breaks[i+1].toFixed(1)}<br>`;
                 }
                 // Class n: > b(n-1) is implicitly covered by the last range shown
             }
        } else {
             // Handle case where breaks calculation failed or resulted in only one value
             div.innerHTML += '<span>Data classification error or single value</span><br>';
        }
        div.innerHTML += '<i style="background:#ccc"></i> No data'; // Style for features with null/invalid data
        return div;
    };
    legend.addTo(map); // Add the updated legend
    ...
    (async () => {
        ...
        raceBorderLayer = L.geoJson(data, {
            style: feature => {
                ...
                return {
                    weight: isSelectedRace ? 1 : 0.25, // Make selected thicker
...
function attachEventListeners() {
    ...
    // --- Add Listeners for Connection Mode Buttons ---
    safeAddListener('show-destinations', 'click', () => {
        if (connectionsMode !== 'destinations') {
            connectionsMode = 'destinations';
            document.getElementById('show-destinations').classList.add('active-connection-mode');
            document.getElementById('show-origins').classList.remove('active-connection-mode');
            if (selectedTractId) { // Update display if a tract is already selected
                displayConnections();
            }
            // Clear table if shown, as data changes
            if (!document.getElementById('floating-output').classList.contains('hidden') && document.getElementById('floating-panel-title').innerText.includes('Connections')) {
                 closeFloatingPanel();
            }
        }
    });
    safeAddListener('show-origins', 'click', () => {
        if (connectionsMode !== 'origins') {
            connectionsMode = 'origins';
            document.getElementById('show-origins').classList.add('active-connection-mode');
            document.getElementById('show-destinations').classList.remove('active-connection-mode');
            if (selectedTractId) { // Update display if a tract is already selected
                displayConnections();
            }
            // Clear table if shown
             if (!document.getElementById('floating-output').classList.contains('hidden') && document.getElementById('floating-panel-title').innerText.includes('Connections')) {
                 closeFloatingPanel();
             }
        }
    ...
    // --- Add Listener for Connection Table Button ---
    safeAddListener('show-connections-table', 'click', showConnectionsTable);


    // --- Index creation buttons ---
    // Reset button
    safeAddListener('reset-selection', 'click', () => {
         clearConnectionsDisplay(); // <<< ADD THIS
         resetIndexSelection(); // Existing reset function
    });
    
    // Generate Index buttons (clear connections before generating)
    safeAddListener('generate-index', 'click', () => {
        clearConnectionsDisplay();
        generateIndex(`${API_BASE_URL}/generate_index`, true) // Using relative path
    });
    safeAddListener('generate-residential', 'click', () => {
        clearConnectionsDisplay(); 
        generateIndex(`${API_BASE_URL}/generate_residential_index`, false) // Using relative path
    });

    // --- Map comparison buttons ---
    // Compare Maps button (clear connections before switching view)
    safeAddListener('compare-maps', 'click', () => {
        clearConnectionsDisplay(); 
        setupComparisonMaps();
    });
    // Back to Single Map button (clear connections state just in case)
    safeAddListener('back-button', 'click', () => {
         clearConnectionsDisplay(); 
         goBackToSingleMap();
    });
